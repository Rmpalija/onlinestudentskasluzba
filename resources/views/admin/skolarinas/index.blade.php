...
@section('content')
    ...
    @can('skolarina_create')
    <p>
        <a href="{{ route('admin.skolarinas.create') }}" class="btn btn-success">@lang('quickadmin.qa_add_new')</a>
    ...
    <p>
        <ul class="list-inline">
            <li><a href="{{ route('admin.skolarinas.index') }}" style="{{ request('show_deleted') == 1 ? '' : 'font-weight: 700' }}">@lang('quickadmin.qa_all')</a></li> |
            <li><a href="{{ route('admin.skolarinas.index') }}?show_deleted=1" style="{{ request('show_deleted') == 1 ? 'font-weight: 700' : '' }}">@lang('quickadmin.qa_trash')</a></li>
        </ul>
    </p>
    @endcan
    ...
    <div class="panel panel-default">
        ...
        <div class="panel-body table-responsive">
            <table class="table table-bordered table-striped {{ count($skolarinas) > 0 ? 'datatable' : '' }} @can('skolarina_delete') @if ( request('show_deleted') != 1 ) dt-select @endif @endcan">
                <thead>
                    <tr>
                        @can('skolarina_delete')
...
@section('javascript') 
    <script>
        @can('skolarina_delete')
            @if ( request('show_deleted') != 1 ) window.route_mass_crud_entries_destroy = '{{ route('admin.skolarinas.mass_destroy') }}'; @endif
        @endcan
